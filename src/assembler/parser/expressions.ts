...
export const bracketedExpr: any = A.coroutine(function* () {
  const states = { OPEN_BRACKET: 0, OPERATOR_OR_CLOSING: 1, ELEMENT_OR_OPENING: 2, CLOSE_BRACKET: 3 };
  ...
  while (true) {
    ...
    if (state === states.OPEN_BRACKET) {
      ...
      if (nextChar === "(") {
        ...
        currentList.push(yield A.choice([hexLiteral, variable]));
        yield A.optionalWhitespace;
        state = states.OPERATOR_OR_CLOSING;
      }
      ...
      currentList.push(yield operator);
      yield A.optionalWhitespace;
      state = states.ELEMENT_OR_OPENING;
    }
  }
...
export const squareBracketExpr = A.coroutine(function* () {
  ...
  while (true) {
    if (expectElement) {
      expr.push(yield A.choice([bracketedExpr, hexLiteral, variable]));
      expectElement = false;
    } else {
      const nextChar = yield peek;
      if (nextChar === "]") {
